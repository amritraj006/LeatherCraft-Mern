  ...
  if (!user) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center p-4 space-y-4">
        ...
        <Link to="/login" className="px-6 py-2.5 rounded-xl bg-walnut text-white font-bold text-xs uppercase tracking-wider hover:bg-walnut/90 transition-all">
          Login now
        </Link>
      </div>
    )
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex items-center justify-between">
        <Link to="/" className="inline-flex items-center gap-1.5 text-xs font-bold text-walnut/60 hover:text-walnut transition-colors uppercase tracking-wider">
      ...
      <div className="p-8 rounded-3xl border border-sand bg-white shadow-sm relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1 relative z-10">
          <span className="text-[10px] font-bold text-terracotta uppercase tracking-widest">In-App Notification Feed</span>
          ...
          <p className="text-xs text-walnut/50 font-semibold">
            Stay updated with real-time status changes, custom creations & transaction invoices.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="p-16 text-center bg-white rounded-3xl border border-sand/40 shadow-sm space-y-3">
            ...
            <p className="text-xs text-walnut/40 font-semibold">
              Any purchase confirmations or product shipping status updates will appear here in real-time.
            </p>
          </div>
        ) : (
            ...
            {notifications.map((n) => (
              <div
                ...
                className={`p-6 transition-colors flex items-start gap-4 cursor-pointer ${
                  n.is_read ? 'bg-white opacity-70 hover:opacity-100' : 'bg-ivory/20 hover:bg-ivory/40'
                }`}
              >
