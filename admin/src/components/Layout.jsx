    ...
    <div className="flex h-screen bg-slate-50/50 overflow-hidden relative font-sans">
      ...
      <header className="flex h-16 items-center justify-between border-b border-sand/30 bg-white/90 backdrop-blur-md px-4 md:hidden fixed top-0 w-full z-30">
        <Link to="/dashboard" className="flex items-center gap-2 text-walnut">
          ...
          <span className="inline-flex items-center gap-1 rounded bg-terracotta/10 px-1 py-0.5 text-[8px] font-semibold uppercase tracking-wider text-terracotta border border-terracotta/20">
            Admin HQ
          </span>
        </Link>
      ...
      >
        <div className="p-6 border-b border-sand/30 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-walnut text-white shadow-md">
              <Scissors size={20} className="rotate-90 text-sand" />
            </span>
            <div>
              <h1 className="text-lg font-serif font-extrabold tracking-tight text-walnut leading-tight">LeatherCraft</h1>
              <span className="inline-flex items-center gap-1 rounded bg-terracotta/10 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wider text-terracotta border border-terracotta/20">
                Admin HQ
              </span>
            </div>
          </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.name}
                to={item.path}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold tracking-wide uppercase transition-all duration-200 ${
                  isActive
          ...
          <div className="flex items-center justify-between gap-3 px-2 py-1">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-xs font-semibold text-walnut truncate leading-none">Admin User</p>
                <p className="text-[10px] font-semibold text-terracotta uppercase tracking-widest mt-1">Superuser</p>
              </div>
            </div>
            <NotificationBell />
          </div>
